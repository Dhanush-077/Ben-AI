...
def run_chat(msg, image=None, media='image/jpeg'):
    dev = __import__('re').compile(r'who.*built').search(str(msg) or '')
    if dev: return ('Built response', [])
    models = [os.environ.get('GEMINI_MODEL','gemini-3.6-flash') or 'gemini-3.6-flash', 'gemini-2.5-flash-lite', 'gemini-flash-latest']
    ...
    for model in uniq:
        ...
        for attempt in range(3):
            ...
            try:
                payload={'contents':[{'role':'user','parts':[{'text':str(msg)}]}], 'system_instruction':{'parts':[{'text':'Be helpful.'}]}}, 'generationConfig':{'temperature':0.7,'maxOutputTokens':2048}}
                ...
                if r.status_code==200 and d.get('candidates'):
                    ...
                    print(f'[ANSWERED] model={model} msg={str(msg)[:20]}', flush=True)
                    return (str(text)[:500], [])
            except Exception as e:
